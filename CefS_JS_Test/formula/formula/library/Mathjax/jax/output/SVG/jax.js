...
(function(AJAX, HUB, HTML, SVG) {
    ...
    //
    var SVGURL = (document.getElementsByTagName("base").length === 0) ? "" : String(document.location).replace(/#.*$/, "");
    SVG.Augment({
        ...
        Startup: function() {
            ...
            BBOX.GLYPH.defs = this.addElement(this.addElement(this.hiddenDiv.parentNode, "svg"), "defs", {
                id: "MathJax_SVG_glyphs"
            });
            // Used in preTranslate to get scaling factors
            this.TestSpan = HTML.Element("span", {
                className: "MathJax_SVG_test"
            }, [
                ["span", {
                    className: "MathJax_SVG_left_box"
                }],
                ["span", {
                    className: "MathJax_SVG_ex_box"
                }],
                ["span", {
                    className: "MathJax_SVG_right_box"
                }]
            ]);
            // Set up styles
        ...
        preTranslate: function(state) {
            ...
            for (i = 0; i < m; i++) {
                ...
                cwidth = Math.max(0, jax.SVG.display ? test.lastChild.offsetWidth - 1 : test.lastChild.offsetLeft - test.firstChild.offsetLeft - 2) / this.config.scale * 100;
                if (ex === 0 || ex === "NaN") {
        ...
        Add: function(svg, dx, dy, forcew, infront) {
            ...
            if (svg.element) {
                ...
                if (Math.abs(svg.x) < 1 && Math.abs(svg.y) < 1) {
                    ...
                    } else if (nodeName === "line" || nodeName === "polygon" || nodeName === "path" || nodeName === "a") {
    ...
    BBOX.TEXT = BBOX.Subclass({
        ...
        Init: function(scale, text, def) {
            ...
            // console.log(scale, SVG.em)
            scale *= 1000 / SVG.em;
            // console.log(SVG.Fixed(scale))
            if(window.location.href.indexOf('RichTextEditor') > -1){//fixed by dongkeying 20190529 适配全等符号以及所有svg绘制的text
                this.element.setAttribute("transform", "scale(" + SVG.Fixed(scale-17) + ") matrix(1 0 0 -1 0 0)");
            }else{
    ...
    HUB.Register.StartupHook("mml Jax Ready", function() {
        ...
        MML.mbase.Augment({
            ...
            SVGaddAttributes: function(svg) {
                ...
                if (this.attrNames) {
                    ...
                    for (var i = 0, m = copy.length; i < m; i++) {
                        var id = copy[i];
                        if (ignore[id] == false || (!skip[id] && !ignore[id] && defaults[id] == null && typeof(svg.element[id]) === "undefined")) {
            ...
            SVGhandleColor: function(svg) {
                ...
                var borders = (this.styles || {}).border,
                    ...
                    id;
                values.background = (this.mathbackground || this.background || (this.styles || {}).background || MML.COLOR.TRANSPARENT);
                if (bleft + pleft) {
                    //
            ...
            SVGnotEmpty: function(data) {
                while (data) {
                    if ((data.type !== "mrow" && data.type !== "texatom") || data.data.length > 1) {
                        return true
                    }
                    data = data.data[0];
                }
            ...
            SVGautoload: function() {
                this.constructor.Augment({
                    toSVG: MML.mbase.SVGautoloadFail
                });
        ...
        MML.mo.Augment({
            toSVG: function(HW, D) {
                ...
                if (SVG.isChar(this.data.join("")) && parent && parent.isa(MML.munderover) && SVG.isChar(this.CoreText(parent.data[parent.base]))) {
        ...
        MML.mrow.Augment({
            ...
            SVGlineBreaks: function(svg) {
                ...
                return (SVG.config.linebreaks.automatic && svg.w > SVG.linebreakWidth) || this.hasNewline();
            },
        ...
        MML.msubsup.Augment({
            toSVG: function(HW, D) {
                ...
                if (this.data[this.base] && (this.data[this.base].type === "mi" || this.data[this.base].type === "mo")) {
                    if (SVG.isChar(this.data[this.base].data.join("")) && base.scale === 1 && !base.stretched && !this.data[this.base].Get("largeop")) {
                        u = v = 0
                    }
                }
        ...
        MML.math.Augment({
            ...
            toSVG: function(span, div) {
                ...
                if (this.data[0]) {
                    ...
                    if (String(HUB.config.displayIndent).match(/^0($|[a-z%])/i)) MML.mbase.prototype.displayIndent = "0";
                    //
                    ...
                    svg.element.setAttribute("viewBox", SVG.Fixed(-l, 1) + " " + SVG.Fixed(-H, 1) + " " + SVG.Fixed(w, 1) + " " + SVG.Fixed(H + D, 1));
                    //
                    ...
                    //
                    if (Math.abs(w - SVG.cwidth) < 10) style.maxWidth = SVG.Fixed(SVG.cwidth * SVG.em / 1000 * SVG.config.scale) + "px";
                    //
        ...
        MML.maligngroup.Augment({
            toSVG: MML.mbase.SVGemptySVG
        });
        MML.malignmark.Augment({
            toSVG: MML.mbase.SVGemptySVG
        });
        MML.mprescripts.Augment({
            toSVG: MML.mbase.SVGemptySVG
        });
        MML.none.Augment({
            toSVG: MML.mbase.SVGemptySVG
