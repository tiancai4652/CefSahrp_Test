...
(function () {
    ...
    var P = (function (prototype, ownProperty, undefined) {
        ...
        return function P(_superclass /* = Object */, definition) {
            ...
            // Set up the prototype of the new class. 原型连
    ...
    function getInterface(v) {
        ...
        var AbstractMathQuill = APIClasses.AbstractMathQuill = P(Progenote, function (_) {
            ...
            //设置光标的位置
            _.positionFix = function (fixPos) {
                this.__controller.textarea.focus();
                if (fixPos == null || fixPos == 0) return;
                var absPos = Math.abs(fixPos);
                for (var i = 0; i < absPos; i++) {
                    if (fixPos > 0) {
                        this.__controller.moveRight();
                    } else {
                        this.__controller.moveLeft();
                    }
                }
            };
            _.html = function () {
        ...
        APIClasses.EditableField = P(AbstractMathQuill, function (_, super_) {
            ...
            _.cmd = function (cmd) {
                var ctrlr = this.__controller.notify(),
                    cursor = ctrlr.cursor;
                    if(cursor.parent&&cursor.parent.jQ[0].className=='mq-text-mode'){
                        return;
                    }
                if (/^\\[a-z]+$/i.test(cmd)) {
                    cmd = cmd.slice(1);
    ...
    var saneKeyboardEvents = (function () {
        ...
        return function saneKeyboardEvents(el, handlers) {
            ...
            target.bind({
                ...
                keyup: onKeyup,
                focusout: onBlur,
                compositionstart: function () { isChinese = true }, //中文输入开始
    ...
    Controller.open(function (_, super_) {
        ...
        _.writeLatex = function (latex) {
            ...
            //限制正体只能输入单个字符从而禁止latex输入
            if($(cursor.parent.jQ[0])[0].className.indexOf("mq-text-mode")!=-1){
                if(latex.length>1) return;
            }
            var all = Parser.all;
            var eof = Parser.eof;
    ...
    var NoSpace = P(Symbol, function (_, super_) {
        _.init = function (ch, html) {
            var str = '<span style="margin-left:-9px">';
            console.log(ch)
            if(ch=='\\!'){
                str = '<span>';
            }
            super_.init.call(this, ch, str + (html || ch) + '</span>');
        };
    });
    ...
    var MathBlock = P(MathElement, function (_, super_) {
        _.join = function (methodName) {
            return this.foldChildren('', function (fold, child) {
                if (fold.indexOf('\\lbrace ') >= 0) {
                   fold = fold.replace(/\\lbrace /g, '\\begin{cases}')
                }
                /*
                    latex数字是一个个取的，fold累加得到最后的数字
                */
                // if ((child[methodName]().substr(0,1) === '_' || child[methodName]().substr(0,1) === '^') && fold.length > 0 && fold.indexOf('\\') < 0 && child[methodName]().indexOf('\\') < 0) {
                //     fold = `{${fold}}`
                // }
                return fold + child[methodName]();
            });
    ...
    var TextBlock = P(Node, function (_, super_) {
        ...
        _.textContents = function () {
            return this.foldChildren('', function (text, child) {
                let str = typeof(child.text) === 'string' ? child.text: child.text();
                return text + str;
            });
        };
        ...
        _.latex = function () {
            ...
            return '\\mathsf{' + contents.replace(/\\/g, '\\backslash ').replace(/[{}]/g, '\\$&') + '}';
        };
        _.html = function () {
            return (
        ...
        _.write = function (cursor, ch) {
            ...
            if (ch !== '$') {
                if (!cursor[L]){
                    TextPiece(ch).createLeftOf(cursor);
                }else{
                    cursor[L].appendText(ch);
                }
        ...
        _.seek = function (pageX, cursor) {
            ...
            if (!cursor.anticursor) {
                ...
                if (this.anticursorPosition === cursorPosition) {
                    ...
                    if (this.anticursorPosition < cursorPosition) {
                        var newTextPc = (typeof cursor[L].splitRight == 'function')&& cursor[L].splitRight(this.anticursorPosition);
                        cursor[L] = newTextPc;
                    } else {
                        var newTextPc = (typeof cursor[L].splitRight == 'function')&& cursor[R].splitRight(this.anticursorPosition - cursorPosition);
                    }
    ...
    // wwm 新增 !
    LatexCmds.nospace = bind(NoSpace, '\\!', '<span></span>');
    ...
    */
    ...
    LatexCmds.leftrightarrows = bind(VanillaSymbol,'\\leftrightarrows','<span style="font-family:Times New Roman;">⇆</span>');
    ...
    // wuwenmei 增加vartriangle \u25b3
    LatexCmds.vartriangle = bind(VanillaSymbol, '\\vartriangle ', '&#x25b3;');
    // VanillaSymbol.apply(this,['measuredangle','&#x23DF;'])
    ...
    LatexCmds.subsetneqq = bind(VanillaSymbol, '\\subsetneqq ', '&#x2ACB;');
    LatexCmds.supsetneqq = bind(VanillaSymbol, '\\supsetneqq ', '&#x2ACC;');
    ...
    LatexCmds.Downarrow = bind(VanillaSymbol, '\\Downarrow ', '&#x21D3;');
    LatexCmds.Uparrow = bind(VanillaSymbol, '\\Uparrow ', '&#x21D1;');
    LatexCmds.overrightarrow = bind(VanillaSymbol, '\\overrightarrow ', '&#x2192;');
    LatexCmds.overleftarrow = bind(VanillaSymbol, '\\overleftarrow ', '&#x2190;');
    LatexCmds.iiiint = bind(VanillaSymbol, '\\iiiint ', '&#x222B;');
    LatexCmds.iiint = bind(VanillaSymbol, '\\iiint ', '&#x222D;');
    LatexCmds.iint = bind(VanillaSymbol, '\\iint ', '&#x222C;');
    LatexCmds.int = bind(VanillaSymbol, '\\int ', '&#x222B;');
    ...
    //lowercase Greek letter variables 小写希腊字母
    LatexCmds.alpha =
        LatexCmds.beta =
        LatexCmds.gamma =
        LatexCmds.delta =
    ...
    LatexCmds.enter = bind(Symbol, '\\enter ', '<var style="display:block"></var>');
    ...
    LatexCmds['\u00bc'] = bind(LatexFragment, '\\frac14');//1/4 分数
    ...
    // wuwenmei
    LatexCmds.overparen = LatexCmds.overgroup = bind(Style, '\\overgroup', 'span', 'class="mq-non-leaf mq-overparen"');
    // hejinjun
    ...
    LatexCmds.underset = P(MathCommand, function (_, super_) {
        _.ctrlSeq = "\\underset";
        _.htmlTemplate = '<span class="mq-underset mq-overunder mq-non-leaf"><span class="mq-over">&1</span><span class="mq-under">&0</span></span>';
        _.textTemplate = ["[", "|", "]"];
    ...
    LatexCmds.overset = P(MathCommand, function (_, super_) {
        _.ctrlSeq = "\\overset";
        _.htmlTemplate = '<span class="mq-overset mq-overunder mq-non-leaf"><span class="mq-over">&0</span><span class="mq-under">&1</span></span>';
        _.textTemplate = ["[", "|", "]"];
        _.finalizeTree = function () {
    ...
    var TextColor = LatexCmds.textcolor = LatexCmds.color = P(MathCommand, function (_, super_) {
    ...
    // wuwenmei---bold
    var TextBold = LatexCmds.boldsymbol = P(MathCommand, function (_, super_) {
        _.setBold = function () {
            this.htmlTemplate =
                '<span class="mq-textcolor" style="font-weight:bold">&0</span>';
        };
        _.latex = function () {
     ...
     //\\overbrace{}^{}
    LatexCmds.overbrace = P(MathCommand,function(_,super_){
        _.ctrlSeq = '\\overbrace{}^{}';
        _.htmlTemplate = 
            '<span class="mq-overunder mq-overset mq-non-leaf" style="margin-right:5px;">'+
            '<span class=" mq-over" style="margin:0;">&0</span>'+
            '<span style="transform:rotate(90deg);display:inline-block;">{</span>'+
            '<span class="mq-overunder mq-underset">'+
            '<span class="mq-under" style="font-size:1em;margin:0;">&1</span>'+
            '</span>'+
            '</span>';
        _.textTemplate = [''];
        _.latex = function(){
            return '\\overbrace{' + this.ends[L].latex() + '}^{' + this.ends[R].latex().substring(1) + '}';
        }
    });
    ...
    LatexCmds.xrightarrow = P(MathCommand,function(_,super_){
        ...
        _.htmlTemplate = 
            '<span class="mq-underset mq-overunder mq-non-leaf" style="vertical-align:-0.2em;">'+
            '<span class="mq-over">&0</span>'+
            '<div class="mq-overarrow mq-arrow-right"></div>'+
            '</span>';
        _.textTemplate = [''];
        _.parser = function() {
            return latexMathParser.optBlock.then(function(optBlock) {
                return latexMathParser.block.map(function(block) {
                    var rarrow = Rarrow();
                    rarrow.blocks = [optBlock, block];
                    optBlock.adopt(rarrow, 0, 0);
                    block.adopt(rarrow, optBlock, 0);
                    return rarrow;
                });
    ...
    LatexCmds['\\xrightarrow[]{}'] = P(MathCommand,function(_,super_){
        _.ctrlSeq = '\\xrightarrow';
        _.htmlTemplate = 
            '<span class="mq-overset mq-overunder mq-non-leaf" style="vertical-align:-0.6em;">'+
    ...
    LatexCmds.xleftarrow = P(MathCommand,function(_,super_){
        ...
        _.htmlTemplate = 
            '<span class="mq-underset mq-overunder mq-non-leaf" style="vertical-align:-0.2em;">'+
            '<span class="mq-over">&0</span>'+
            '<div class="mq-overarrow mq-arrow-left"></div>'+
            '</span>';
        _.textTemplate = [''];
        _.parser = function() {
            return latexMathParser.optBlock.then(function(optBlock) {
                return latexMathParser.block.map(function(block) {
                    var larrow = Larrow();
                    larrow.blocks = [optBlock, block];
                    optBlock.adopt(larrow, 0, 0);
                    block.adopt(larrow, optBlock, 0);
                    return larrow;
                });
            }).or(super_.parser.call(this));
        };
    ...
    // LatexCmds['\\overset{}{\\longleftrightarrow}'] = P(MathCommand,function(_,super_){
    //     _.ctrlSeq = '\\overset{}{\\longleftrightarrow}';
    //     _.htmlTemplate = 
    //         '<span class="mq-underset mq-overunder mq-non-leaf" style="vertical-align:-0.2em;">'+
    //         '<span class="mq-over">&0</span>'+
    //         '<div class="mq-overarrow mq-arrow-both"></div>'+
    //         '</span>';
    //     _.latex = function(){
    //         return '\\overset{' + this.ends[L].latex() + '}{\\longleftrightarrow}';
    //     }
    // });
    //\\underset{}{\\longleftrightarrow}
    // LatexCmds['\\underset{}{\\longleftrightarrow}'] = P(MathCommand,function(_,super_){
    //     _.ctrlSeq = '\\underset{}{\\longleftrightarrow}';
    //     _.htmlTemplate = 
    //         '<span class="mq-underset mq-overunder mq-non-leaf" style="vertical-align:-0.6em;">'+
    //         '<div class="mq-overarrow mq-arrow-both"></div>'+
    //         '<span class="mq-under">&0</span>'+
    //         '</span>';
    //     _.latex = function(){
    //         return '\\underset{' + this.ends[L].latex() + '}{\\longleftrightarrow}';
    //     }   
    // });

    // //\\underset{}{\\overset{}{\\longleftrightarrow}}
    // LatexCmds['\\underset{}{\\overset{}{\\longleftrightarrow}}'] = P(MathCommand,function(_,super_){
    //     _.ctrlSeq = '\\underset{}{\\overset{}{\\longleftrightarrow}}';
    //     _.htmlTemplate = 
    //         '<span class="mq-overset mq-overunder mq-non-leaf" style="vertical-align:-0.7em;">'+
    //         '<span class="mq-over">&0</span>'+
    //         '<div class="mq-overarrow mq-arrow-both"></div>'+
    //         '<span class="mq-overunder mq-underset">'+
    ...
    //         '</span>';
    //     _.latex = function(){
    //         return '\\underset{' + this.ends[R].latex() + '}{\\overset{' + this.ends[L].latex() + '}{\\longleftrightarrow}}';
    //     }
    // });
    ...
    var SupSub = P(MathCommand, function (_, super_) {
        ...
        _.latex = function () {
            ...
            // \lbrace公式特殊处理 -- wuwenmei
            if (this['-1'].ctrlSeq === '\\lbrace ') {
                return latex('', this.sup) + '\\\\' + latex('', this.sub) + '\\\\\\end{cases}'
            }
            // --end
            return latex('_', this.sub) + latex('^', this.sup); // 上下标符号 +{}
        };
    ...
    // LatexCmds['\\\\'] =
    // wwm z增加mathop
    LatexCmds['mathop'] = LatexCmds['^'] = P(SupSub, function (_, super_) {
            _.supsub = 'sup';
            _.htmlTemplate =
    ...
    var SummationNotation = P(MathCommand, function (_, super_) {
        ...
        _.latex = function () {
            function simplify(latex) {
                return '{' + (latex || ' ') + '}'; // --- wuwenmei return latex.length === 1 ? latex : '{' + (latex || ' ') + '}';
            }
    ...
    // // baolilei 一
    // var Underbrace = P(MathCommand, function (_, super_) {
    //     // _.ctrlSeq = '\\underbrace{}_{\\text{}}';
    //     _.init = function (ch, html) {
    //         var htmlTemplate =
    //             '<span class="mq-large-operator mq-non-leaf">' +
    //             '<span class="mq-to"><span>&1</span></span>' +
    ...
    //             '_{' + simplify(this.ends[R].latex())+'}}').replace(/_{ }/,'');
    //     };
    //     // _.textTemplate=[this.ends[L].latex(),this.ends[R].latex()];
    //     _.parser = function () {
    //         var string = Parser.string;
    //         var optWhitespace = Parser.optWhitespace;
    //         var succeed = Parser.succeed;
    ...
    //             blocks[i].adopt(self, self.ends[R], 0);
    //         }
    //         return optWhitespace.then((string('_{\\text{')).or(string('_{\\text{'))).then(function (supOrSub) {
    //             var child = blocks[supOrSub === '_{\\text{' ? 0 : 1];
    //             return block.then(function (block) {
    //                 block.children().adopt(child, child.ends[R], 0);
    //                 return succeed(self);
    //             });
    ...
    // });
    // LatexCmds['\u23df'] = 
    // LatexCmds.underbrace = bind(Underbrace, '\\underbrace ', '&#x23DF;');
  

    // baolilei
    var Underbrace = P(MathCommand,function(_,super_){
        _.ctrlSeq = '\\underbrace';
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-underbrace mq-overset mq-non-leaf" style="vertical-align:-2em;margin-left: 6px;margin-right:5px;">'+
                '<span class=" mq-over" style="font-size:1em;margin:0;">&0</span>'+
                '<span style="transform:rotate(270deg);display:inline-block;">{</span>'+
                '<span class="mq-underbrace mq-underset">'+
                '<span class="mq-under" style="margin:0;">&1</span>'+
                '</span>'+
                '</span>';
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };  
        _.latex = function(){
            return this.ctrlSeq+ '{' + this.ends[L].latex() + '}_' + this.ends[R].latex().substring(1);
        }
    });
   ...
   LatexCmds.underbrace = bind(Underbrace, '\\underbrace ', '&#x23DF;');
    ...
    // hejinjun ---------------------------------------------------------- 
    // 
    // function getIntTemplate(num) {
    //     var big = '';
    //     for (var i = 0; i < num; i++) {
    //         big += '<big>&int;</big>'
    //     }
    //     var htmlTemplate =
    //         '<span class="mq-int mq-non-leaf">' + big +
    //         '<span class="mq-supsub mq-non-leaf">' +
    ...
    //         _.init = function() {
    //             Symbol.prototype.init.call(this, '\\int ', getIntTemplate(1));
    //         };
    ...
    //     _.init = function() {
    //         Symbol.prototype.init.call(this, '\\iint ', getIntTemplate(2));
    //     };
    //     // FIXME: refactor rather than overriding
    //     _.createLeftOf = MathCommand.p.createLeftOf;
    // });
    ...
    //     _.init = function() {
    //         Symbol.prototype.init.call(this, '\\iiint ', getIntTemplate(3));
    //     };
    //     // FIXME: refactor rather than overriding
    //     _.createLeftOf = MathCommand.p.createLeftOf;
    // });
    ...
    // var DotNotation = P(MathCommand, function(_, super_) {
    //     _.init = function(ch) {
    //         Symbol.prototype.init.call(this, ch, '<span class="mq-non-leaf"><span class="mq-dot-recurring-inner"><span class="mq-dot-recurring">&#x2d9;</span><span class="mq-empty-box">&0</span></span></span>')
    //     }
    // });


    // LatexCmds.dot = bind(DotNotation, "\\dot");
    ...
    var SquareRoot =
        ...
        LatexCmds['\u221a'] = P(MathCommand, function (_, super_) {
            ...
            _.reflow = function () {
                ...
                var y = block.innerHeight() / +block.css('fontSize').slice(0, -2) - .1;
                if(y<0) y=1;
                scale(block.prev(), 1, y);
            };
        });

    var Hat = LatexCmds.hat =LatexCmds.widehat = P(MathCommand, function(_, super_) {
        _.ctrlSeq = '\\hat';
        _.htmlTemplate =
            '<span class="mq-non-leaf">' +
        ...
        _.matchBrack = function (opts, expectedSide, node) {
            ...
            return node instanceof Bracket && node.side && node.side !== -expectedSide &&
                (!opts.restrictMismatchedBrackets ||
                    OPP_BRACKS[this.sides[this.side].ch] === node.sides[node.side].ch || { '(': ']', '[': ')' }[this.sides[L].ch] === node.sides[R].ch) && node;
        };
    ...
    LatexCmds.left = P(MathCommand, function(_) {
        _.parser = function() {
            ...
            return optWhitespace.then(regex(/^(?:[\]\.\)([|]|\\\{|\\\||\\langle|\\lVert\b)/))
                .then(function(ctrlSeq) {
                    ...
                    return latexMathParser.then(function(block) {
                        return string('\\right').skip(optWhitespace)
                            .then(regex(/^(?:[\[\]\.)|]|\\\}|\\rangle|\\rVert\b)/)).map(function(end) {
    ...
    //\embed{registeredName} lateX like this
    ...
    var Environment = P(MathCommand, function(_, super_) {
        ...
        _.wrappers = function (value) {
            if (value) _.template[0][1] = '}{' + value + '} ';
            return [
    ...
    //array模式
    Environments.array = P(Environment, function(_, super_) {
        ...
        _.latex = function() {
            ...
            return this.wrappers(delimiters.alignResult).join(latex);
        };
        ...
        _.html = function() {
            ...
            this.eachChild(function (cell) {
                ...
                cells[row].push('<td class="">&'+(i++)+'</td>');
            });

            this.htmlTemplate =
                    '<span class="mq-matrix mq-non-leaf">'
                +   parenHtml(this.parentheses.left)
                +   '<table class="mq-non-leaf" cellspacing="10">'
        ...
        _.parser = function() {
            ...
            delimiters.alignResult = items[0].ends[1].ctrlSeq;
            //array模式下items[0]为对齐方式的值，需要去掉
    ...
    Environments.cases = P(Environments.array, function(_, super_) {
        _.environment = 'cases';
        _.parentheses = {
            left: '{',
            right: ''
    ...
    var matrixEnvironment = P(MathCommand, function(_, super_) {
        _.template = [['\\begin{', '}'], ['\\end{', '}']];
        _.wrappers = function () {
            return [
            _.template[0].join(this.environment),
    ...
    Environments.matrix = P(matrixEnvironment, function(_, super_) {
